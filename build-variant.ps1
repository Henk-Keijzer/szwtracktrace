...
# 1. een main.dart met daarin de baseURL van de variant (de server, bijvoorbeeld https://tt.zeilvaartwarmond.nl/ (dus met afsluitende slash))
...
#    >keytool -genkey -v -keystore variants/xxx/upload-keystore.jks -storetype JKS -keyalg RSA -keysize 2048 -validity 10000 -alias upload
...
# replace the manifest.json file
...
# remove the icon folder from a previous run and rename the icons folder to icon (webservers often use icons for their own porpose)
Remove-Item -Path "release\$variant\web\icon\*" -Recurse
Remove-Item -Path "release\$variant\web\icon"
Rename-Item "release\$variant\web\icons" "icon"

# build the Android App Bundle
flutter build appbundle
# copy the result to the release folder
