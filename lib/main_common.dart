...
Map<String, dynamic> dirList = {}; // see /get?req=dirlist on the server
List<String> eventNameList = [];
List<String> eventYearList = [];
List<String> eventDayList = [];
Map<String, dynamic> eventInfo = {}; // see /get?req=eventinfo&event=event/year/day on the server
String eventDomain = '';
String eventId = '';
...
int sliderEnd = 0; // end of the time slider, during live it is the current time minud displaydelay, during replay it is eventend
...
extension ResetTicker on Ticker {
  void reset() {
    stop();
    start();
  }
...
// Note that the powershell script build-variant.ps1 copies the main.dart specifically for the variant from the variants folder/xxx
//
...
void mainCommon({required String serverUrl}) async {
  ...
  // and split them up over the three types of maps: base, overlay and labels
  ...
  selectedMapType = (kIsWeb && Uri.base.queryParameters.containsKey('map'))
      ? Uri.base.queryParameters['map'].toString() //get parameter with attribute "map"
      : selectedMapType; // or keep the same value
  ...
  if (overlayTileProviders.isEmpty) {
    ...
    // does the list contain the overlaymaptype from a previous session
...
class MyAppState extends State<MyApp> with WidgetsBindingObserver, SingleTickerProviderStateMixin {
  ...
  // The UI is rebuilt each time the info to be displayed needs to be updated to the screen.
  //
  ...
  Widget build(BuildContext context) {
    ...
    // we need to calculate this because in the Scaffold below, we extend the map behind the appBar and the notification area
  ...
  //----------------------------------------------------------------------------
  //
  // the UI elements listed in the scaffold-stack above. The names speak for themselves (I hope)
  //
  AppBar uiAppBar() {
    return AppBar(
    ...
    return FlutterMap(
        ...
        options: MapOptions(
            ...
            onPositionChanged: (_, gesture) {
              ...
              if (gesture) {
                setState(() {
                  // stop autozoom and autofollow, close all menu's
            ...
            onTap: (_, __) => setState(() {
                  ...
                  showEventMenu =
                      showMapMenu = showShipMenu = showPreEventParticipants = showInfoPage = showAttribution = showTestingMenu = false;
                })),
        children: [
          // eight children: the base map, the optional labeloverlay for base (satellite) maps,
          // the (selectable) overlay map, the windParticles, the scale bar, the route polylines, the route polygons and finally all
          // markers/textlabels
          //
          ...
          Scalebar(
              ...
              strokeWidth: 1),
          //
          // three more layers: the 6. lines of the route and trails, 7. the fences and 8. the markers/labels
  ...
  Column uiSliderArea() {
    return Column(mainAxisAlignment: MainAxisAlignment.end, crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      // under this row 2. the start/stop button and the replayslider
      uiReplaySlider(),
  ...
  Container uiSpeedSlider() {
    Color sliderColor = (bgColor == bgDark) ? Colors.black54 : Colors.white60;
    Color thumbColor = (bgColor == bgDark) ? Colors.black54 : Colors.white;
    return Container(
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.end, crossAxisAlignment: CrossAxisAlignment.center, children: [
          IconButton(
              ...
              tooltip: "sneller",
              onPressed: () => setState(() => speedIndex = (speedIndex == speedTable.length - 1) ? speedIndex : speedIndex + 1)),
          RotatedBox(
            quarterTurns: 3,
            child: SliderTheme(
    ...
    return Column(mainAxisAlignment: MainAxisAlignment.end, children: [
      ...
      if (autoFollow)
        Text('zoom',
            ...
            style: TextStyle(fontSize: 13, height: 0.01, color: (bgColor == bgDark) ? Colors.black : Colors.white)),
  ...
  Container uiSliderText() {
    Color textColor = (bgColor == bgDark) ? Colors.black : Colors.white;
    return Container(
        ...
        Column(children: [
          ...
          Container(
              ...
              child: Column(children: [
                ...
                if (!kIsWeb || document.referrer == '') // no sharing possible from <iframe> embedded mode
                  IconButton(
                      onPressed: () async {
    ...
    return SingleChildScrollView(
        child: Container(
            ...
            child: Column(children: [
                ...
                itemBuilder: (_) {
                  return eventNameList
                      .map((event) => PopupMenuItem(
                          height: 25,
                          value: event,
                          child: Text(event, style: TextStyle(color: (event == eventName) ? Colors.deepOrange : Colors.black))))
                  ...
                  itemBuilder: (_) {
                    return eventYearList
                        .map((year) => PopupMenuItem(
                            height: 25,
                            value: year,
                            child: Text(year, style: TextStyle(color: (year == eventYear) ? Colors.deepOrange : Colors.black))))
                        .toList(growable: false);
                  },
                  ...
                  itemBuilder: (_) {
                    return eventDayList
                        .map((dayRace) => PopupMenuItem(
                            height: 25,
                            value: dayRace,
                            child: Text(dayRace, style: TextStyle(color: (dayRace == eventDay) ? Colors.deepOrange : Colors.black))))
                        .toList(growable: false);
                  },
              ...
              if (selectionMessage != '') Wrap(children: [const Divider(), Text(selectionMessage)]),
              if (eventDomain != '')
                Wrap(children: [
                  const Divider(),
                  Container(
                    ...
                    Row(children: [
                      ...
                      Checkbox(
                          ...
                          onChanged: (value) => setState(() {
                                ...
                                updateGPSBuoys(currentReplayTime);
                                prefs.setBool('routelabels', showRouteLabels); // (don't await the result...)
                              }))
                    ])
                  ]),
      ...
      Column(mainAxisAlignment: MainAxisAlignment.end, crossAxisAlignment: CrossAxisAlignment.end, children: [
        ...
        IconButton(
          icon: (showAttribution)
              ? Icon(Icons.cancel_outlined, color: (bgColor == bgDark) ? Colors.black38 : Colors.white60)
              : Icon(Icons.info_outline, color: (bgColor == bgDark) ? Colors.black38 : Colors.white60),
  ...
  void startPreEvent() async {
    ...
    // see if there are already participants in the database (will be loaded in shipList, shipColors and teamList)
    await getParticipants(eventDomain);
    // see if we have a route
    if (route['features'] != null) {
  ...
  // 2. the live timer routine, runs every hfUpdateInterval (=100) milliseconds
  //
  ...
  // During live with prediction and replay this routine is called 20 times per second, so in that situation the routine is time critical
  //
    ...
    for (int i = 0; i < replayTracks['shiptracks'].length; i++) {
      ...
      if (shipTrack['stamp'].length > 0) {
        ...
        shipLabelList[i] = showShipLabels
            ? mapTextLabel(
                calculatedPosition,
                '${showTeam ? teamList[i] : shipList[i]}'
                '${shipLostSignalIndicators[i]}'
                '${(showShipSpeeds ? ', $speedString' : '')}')
      ...
      if (!showRoute) {
        ...
        if (gpsBuoy['stamp'].length > 0) {
          ...
          if (gpsBuoyTimeIndex[i] != timeIndex) {
            ...
            // now add the buoy marker
            String svgString = '<svg width="22" height="22"><circle cx="11" cy="11" r="3.5" '
  ...
  // Note that there may be more/new ships/buoys/windstations in liveTrails then in replayTracks, because a ship may have joined
  ...
  void setupShipGpsBuoyAndWindInfo() {
    ...
    shipTrailList = List.filled(nrShips, Polyline(points: [const LatLng(0, 0)]), growable: true);
    shipLostSignalIndicators = List.filled(nrShips, '', growable: true);
    shipList = [];
    teamList = [];
    shipColors = [];
    shipColorsSvg = [];
  ...
  //
  Future<void> getParticipants(domain) async {
    shipList = [];
    teamList = [];
    shipColors = [];
    ...
    for (var participant in participants) {
      shipColors.add(Color(shipMarkerColorTable[int.parse(participant['colorcode']) % 32]));
      shipList.add(participant['shipname']);
      teamList.add(participant['teamname'] != '' ? participant['teamname'] : 'Team ${participant['shipname']}');
    }
  }
  ...
  Map<String, dynamic> convertTimes(track) {
    ...
    track['endtime'] *= 1000;
    track['shiptracks'] ??= [];
    track['shiptracks'] = track['shiptracks'].map((ship) {
      ship['stamp'] = ship['stamp'].map((timeStamp) => timeStamp * 1000).toList();
      return ship;
    }).toList();
    track['buoytracks'] ??= [];
    track['buoytracks'] = track['buoytracks'].map((buoy) {
      buoy['stamp'] = buoy['stamp'].map((timeStamp) => timeStamp * 1000).toList();
      return buoy;
    }).toList();
    track['windtracks'] ??= [];
    track['windtracks'] = track['windtracks'].map((station) {
      station['stamp'] = station['stamp'].map((timeStamp) => timeStamp * 1000).toList();
      return station;
    }).toList();
    return track;
  }
  ...
  ({int heading, double speed}) centerWind(windTracks, nrStations) {
    ...
    final Map<int, double> unsortedDistanceTable = {}; // map with station index in the windTracks as key and the distance as value
    final distance = const Distance();
    final List<double> eastWestVectors = [];
    final List<double> northSouthVectors = [];
    final List<double> distanceFactors = [];
    ...
    for (int i = 0; i < windTracks.length; i++) {
      unsortedDistanceTable[i] = distance.as(
        LengthUnit.Meter,
        mapController.camera.center,
        LatLng(windTracks[i]['lat'][0].toDouble(), windTracks[i]['lon'][0].toDouble()),
      );
    }
    // sort the map from near to far (values)
    final distanceTable = unsortedDistanceTable.entries.toList()..sort((e1, e2) => e1.value.compareTo(e2.value));
    ...
    for (int i = 0; i < nrStations; i++) {
      ...
      final windDirection = (windTracks[distanceTable[i].key]['course'][windTimeIndex[distanceTable[i].key]] * pi / 180).toDouble();
      final windSpeed = windTracks[distanceTable[i].key]['speed'][windTimeIndex[distanceTable[i].key]].toDouble();
      eastWestVectors.add(windSpeed * cos(windDirection));
      northSouthVectors.add(windSpeed * sin(windDirection));
    }
    // Normalize the 'reversed' distance factors to 1.0 and calculate the weighted vector
    final double normalizedSumDistances = distanceFactors.reduce((a, b) => a + b);
    final normalizedDistanceFactors = distanceFactors.map((factor) => factor / normalizedSumDistances).toList();

    for (int i = 0; i < nrStations; i++) {
      eastWestSum += eastWestVectors[i] * normalizedDistanceFactors[i];
      northSouthSum += northSouthVectors[i] * normalizedDistanceFactors[i];
    }

    // Return the result (heading always from 0-359)
    return (
